...
def main() -> None:
    ...
    try:
        while True:
            ...
            if out_writer is None and args.output:
                h, w = frame_bgr.shape[:2]
                out_writer = cv2.VideoWriter(args.output, fourcc, cap.get(cv2.CAP_PROP_FPS) or 30.0, (w, h))
            ...
            annotated = drawer.annotate(frame_bgr, result, hud) if not args.no_draw else frame_bgr

            if args.output and out_writer is not None:
                out_writer.write(annotated)

            cv2.imshow("Fitness Pose 3D (Video)", annotated)
